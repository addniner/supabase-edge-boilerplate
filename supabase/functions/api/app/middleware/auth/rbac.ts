 ...
 *
 * @example
 * app.post("/upload", requireAnyPermission([Permission.IMAGES_GENERATE, Permission.ADMIN]), async (c) => {
 *   // 둘 중 하나라도 있으면 실행
 * });
 */
 ...
 *
 * @example
 * app.post("/dangerous", requireAllPermissions([Permission.PROJECTS_DELETE, Permission.ADMIN]), async (c) => {
 *   // 모든 권한이 있어야 실행
 * });
 */
